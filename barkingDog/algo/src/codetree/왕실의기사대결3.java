...
public class 왕실의기사대결3 {
	...
	public static void main(String[] args) throws IOException{
		...
		map = new int[l+2][l+2];
		for(int i = 0 ; i < l+2 ; i++) Arrays.fill(map[i], 2);
	...
	static void bfs(int start, int dir) {
		...
		while (!q.isEmpty()) {
			...
			//전체 기사 중 겹치면 q 에 삽입
			for(int i =1 ; i <= n ; i++) {
				if(vis[i] || isDead[i]) continue;
				
				Unit o = units[i];
				
				if(nr >= o.r + o.h || nr +cur.h <= o.r || nc >= o.c + o.w || nc+cur.w <= o.c) continue;
				q.offer(i);
				vis[i] = true;
			}
		}
		...
		//전체기사 중에 사라짐 처리 혹은 이동 처리
		for(int i = 1; i<= n ;i++) {
			if(isDead[i] || !vis[i]) continue;
			
			Unit u = units[i];
			
			if(u.k - damage[i] <= 0) isDead[i] = true;
			else {
				u.r += dr[dir];
				u.c += dc[dir];
				u.k -= damage[i];
