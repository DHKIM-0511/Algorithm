...
public class 보호필름 {
	...
	private static void minChange(int idx , int tmp) {
		//성능이 통과인지 아닌지 판단.
		if(isPass()) {
			...
			for(int i = idx ; i < d ; i++) {
				int[] copy = new int[w];
				for(int j = 0 ; j < w ;j++) copy[j] = map[idx][j];
				
				tmp++;
				for(int j = 0 ; j < w ;j++) map[idx][j] = 0;
				minChange(i+1 , tmp); //A투입 후 다시 검사
				for(int j = 0 ; j < w ;j++) map[idx][j] = copy[j];
				
				for(int j = 0 ; j < w ;j++) map[idx][j] = 1;
				minChange(i+1 , tmp); //B투입 후 다시 검사
				for(int j = 0 ; j < w ;j++) map[idx][j] = copy[j];
